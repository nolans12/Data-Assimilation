...
class BallisticTarget:
    ...
    def __init__(
        ...
        # Compute parabolic coefficients for altitude profile
        # We want: alt(0) = alt0, alt(0.5) = max_alt, alt(1) = alt1
        # Use quadratic: alt(t_norm) = a*t_norm^2 + b*t_norm + c
        # where t_norm = t / duration (normalized time in [0, 1])
        self.alt_c = self.alt0
        # Solve system:
        # c = alt0
        # a + b + c = alt1
        # 0.25*a + 0.5*b + c = max_alt
        # => a + b = alt1 - alt0
        # => 0.25*a + 0.5*b = max_alt - alt0
        # From second equation: b = alt1 - alt0 - a
        # Substitute: 0.25*a + 0.5*(alt1 - alt0 - a) = max_alt - alt0
        # => 0.25*a + 0.5*alt1 - 0.5*alt0 - 0.5*a = max_alt - alt0
        # => -0.25*a = max_alt - alt0 - 0.5*alt1 + 0.5*alt0
        # => -0.25*a = max_alt - 0.5*alt0 - 0.5*alt1
        # => a = -4*(max_alt - 0.5*alt0 - 0.5*alt1)
